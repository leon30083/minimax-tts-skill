...
# 配置
SCENES_FILE = Path(__file__).parent.parent / "guangpan_scenes_semantic.yaml"
OUTPUT_DIR = Path(__file__).parent.parent / "outputs" / "guangpan_semantic"
...
def generate_story(voice_id: str = "Chinese (Mandarin)_Warm_Girl", model: str = "speech-2.6-turbo"):
    ...
    print("="*70)
    print("🎵 《光盘行动》绘本音频生成（语义分析版）")
    print("="*70)
    print(f"\n🎭 音色: {voice_id}")
    print(f"🤖 模型: {model}")
    print(f"📊 场景数: {len(scenes)}")
    print(f"✨ 特点: 语义停顿 + 情感语速调整\n")
    ...
    for r in results:
        if r.get("success"):
            print(f"   场景 {r['scene']:02d} [{r['tone']}]: MEDIA:skills/minimax-tts/outputs/guangpan_semantic/scene_{r['scene']:02d}.mp3")
